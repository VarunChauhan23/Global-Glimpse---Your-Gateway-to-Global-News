...
class News extends Component {
    ...
    try {
      ...
      this.setState((prevState) => ({
        articles:
          page === 1
            ? parseddata.articles
            : prevState.articles.concat(parseddata.articles),
  ...
  handleSearch = async (event) => {
    ...
    const matchedKey = Object.keys(this.countries).find((key) => {
      const values = Array.isArray(this.countries[key])
        ? this.countries[key]
        : [this.countries[key]];
      return (
        key === this.state.searchTerm.toLowerCase() ||
        values
          .map((v) => v.toLowerCase())
    ...
    if (matchedKey) {
      ...
      notify(
        "Sorry, invalid country or we do not cover this country yet",
        "error",
  ...
  render() {
    return (
      <>
        ...
        >
          Global Glimpse - Top {this.capitalizeFirstLetter(this.props.category)}{" "}
          Headlines
        </h1>
        <InfiniteScroll
